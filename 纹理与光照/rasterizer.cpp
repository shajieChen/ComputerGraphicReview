...
{ 
    ...
    {  
        ...
        if(v[i].x() <= minWidth)
        {
            minWidth = floor(v[i].x()) ; 
        }
        if(v[i].y() >= maxHeight)
        {
            maxHeight = ceil(v[i].y());
        } 
        if(v[i].y() <= minHeight)
        {
            minHeight = floor(v[i].y()); 
        }
    } 

    for(int i = (int)minWidth; i < (int)maxWidth ; i ++ )
    {
        for(int j = (int)minHeight; j < (int)maxHeight ; j++ )
        {
            float x = i + 0.5 ;  
            float y = j + 0.5 ;          
            if(insideTriangle(x , y , t.v))
            {
                ...
                if(zp < depth_buf[get_index(i , j )])
                {
                    ...
                    auto pixel_color = fragment_shader(payload); //Final Color
